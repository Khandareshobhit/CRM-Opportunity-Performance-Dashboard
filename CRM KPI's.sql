...
##
#### KPI FOR LEAD DASHBOARD
##

# 1) Total Lead

SELECT SUM(`Total Leads`)  AS Total_Lead
FROM `lead`; 
...
SELECT 
	CONCAT(
			ROUND(
					SUM(`Expected Amount`)/1000000
			   ,2)
	  ,"M") AS Expected_Amount_From_Converted_Leads
FROM `opportunity table` T1 
JOIN `lead` T2 ON T1.`Opportunity ID` = T2.`Converted Opportunity ID`
WHERE Converted = "TRUE";

# 3) Converted Accounts

SELECT 
		SUM(`# Converted Accounts`) AS No_Converted_Accounts
FROM `lead`;

# 4) Converted Opportunities

SELECT 
		SUM(`# Converted Opportunities`) AS No_Converted_Opportunities
FROM `lead`; 
...
SELECT
		`Lead Source`, 
        SUM(`Total Leads`) AS "Lead"
FROM `lead`
GROUP BY `Lead Source`
ORDER BY SUM(`Total Leads`)  DESC;
...
SELECT
        ...
        SUM(`Total Leads`) AS "Lead"
FROM `lead` T1 
JOIN `opportunity table` T2 ON T1.`Converted Opportunity ID` = T2.`Opportunity ID`
GROUP BY Stage
ORDER BY SUM(`Total Leads`) DESC;
